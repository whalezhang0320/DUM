...
from lifelong_rl.models.density.flow_gan import ConditionalFlow
from lifelong_rl.util.schedule import ConstantSchedule
...
class SACTrainer(TorchTrainer):
    ...
    def __init__(
        ...
        self.use_density_penalty = use_density_penalty
        self.density_random_num_samples = density_random_num_samples
        self.density_ucb_ratio = density_ucb_ratio
        self.density_base_weight_schedule = ConstantSchedule(density_base_weight)
        self.density_nll_clip_min = density_nll_clip_min
        self.density_nll_clip_max = density_nll_clip_max
        self.density_model = None

        if self.use_density_penalty:
            if not hasattr(self.env.observation_space, 'shape') or not hasattr(self.env.action_space, 'shape'):
                raise ValueError('Density penalty requires continuous observation/action spaces')

            state_dim = int(np.prod(self.env.observation_space.shape))
            action_dim = int(np.prod(self.env.action_space.shape))
            if flow_model_path:
                self.density_model = ConditionalFlow(action_dim, state_dim).to(ptu.device)
                state_dict = torch.load(flow_model_path, map_location=ptu.device)
                self.density_model.load_state_dict(state_dict)
    ...
    def train_from_torch(self, batch, indices):
        ...
        if self._need_to_update_eval_statistics:
            ...
            for key, value in density_logs.items():
